...
完整且可直接加载的示例请查看 [`score/谱面模板.param`](score/谱面模板.param)。该文件通过注释说明了每个字段，并演示了普通按键、组合键、拍内连续动作、休止、多个动作块和横笛颤音的写法。

- `instrument`：游戏内乐器编号。
- `1464750897`：WarmNote 谱面的固定格式魔数，不要修改。
- `1 1 1`：依次表示格式版本、段落数量和记录总数；增删内容时必须同步更新。
